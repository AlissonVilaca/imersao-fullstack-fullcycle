...
import (
	...
	"github.com/AlissonVilaca/imersao/codepix-go/domain/model"
...
func (r PixKeyRepositoryDb) FindKeyByKind(key string, kind string) (*model.PixKey, error) {
	...
	r.Db.Preload("Account.Bank").First(&pixKey, "kind = ? and key = ?", kind, key) // esse Preloadtraz todo o resultado do banco relacionado a pixkey, os atributos da pixkey tambem sao preenchidos
